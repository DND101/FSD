<hr class = "footerline"><!--css modified horizontal line-->
<footer>
    <div class = "container">
        <div class = "row">
            <section>
                <div class = "footerContent col-md-4"><!--left content-->
                    <p class = "footerContent1">
                        <strong>M</strong><span class = "small footerSubtext">aharashtra</span>
                        <strong>I</strong><span class = "small footerSubtext">nstitute of</span>
                        <strong>T</strong><span class = "small footerSubtext">echnonolgy</span>
                        <strong>P</strong><span class = "small footerSubtext">UNE</span>
                    </p>

                    <p class = "footerSubtext2">
                    Our college Mission is to impart quality technical education and higher moral ethics associated with skilled training to suit the modern day technology with innovative concepts,so as to learn to lead the future with full confidence
                  .
                    </p>
                </div>
            </section>
            <section>
                <div class = "footcontent col-md-4"><!--middle content-->
                <p> <br>
                 Address: MIT WPU UNIVERSITY,
                    PAUD ROAD
                    <br>PUNE, MAHARASHTRA 411038 </p>
                </div>
            </section>
            <section>
                <div class = "footcontent col-md-4"><!--right content-->
                    Follow Us:<br>
                    <a href="https://www.facebook.com/MITWPUOfficial//"><img src = "images/facebook.png"></a>
                    <a href="https://www.instagram.com/mitwpuofficial/?hl=en/"><img src = "images/instagram.png" width="65px"></a>
                        
                        <a href="https://www.youtube.com/c/mitworldpeaceuniversity"><img src = "images/youtube.png"></a>
                </div>
            </section>
        </div>
    </div>
</footer>